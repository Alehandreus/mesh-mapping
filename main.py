...
from utils import sample_points, chamfer_distance
...
def main():
    ...
    while len(rough_mesh.get_vertices()) < 300000: # subdivide each primitive until we have enough vertices
        rough_mesh.split_faces(0.5)
    rough_sampler = GPUMeshSampler(rough_mesh, MeshSamplerMode.SURFACE_UNIFORM, max(batch_size, save_points))

    fine_mesh = Mesh.from_file(fine_path)
    fine_sampler = GPUMeshSampler(fine_mesh, MeshSamplerMode.SURFACE_UNIFORM, max(batch_size, save_points))
    ...
    for it in range(1, iters + 1):
        ...
        if it % log_interval == 0:
            points_true = sample_points(fine_sampler, save_points, device)
            points_mapped = sample_points(rough_sampler, save_points, device)
